...
def sample_from_array(array, traj_shape):

	num_samples = traj_shape[0]
	sample = np.zeros(traj_shape)
	for i in range(array.shape[1]):
		target = np.abs(array[1][i]-array[0][i])*np.random.rand(num_samples)+min(array[1][i],array[0][i])
		target = target.reshape((num_samples,1))
		if i == 0:
			target_set = target
		else:
